...
const ResetPassword = () => {
  ...
  const onSubmitOTP = async (e) => {
    ...
    // Ensure all refs are valid before accessing them
  ...
  const onSubmitNewPassword = async (e) => {
    ...
    const enteredOtp = inputRefs.current
      .map((input) => input?.value || "") // Ensure input is not null
      .join("");
    ...
    try {
      ...
      console.log("This is error is in the data",data)
      if (data.success) {
            ...
            {Array(6)
              ...
              .map((_, index) => (
                <input
                  ...
                  key={index}
                  required
                  className="w-12 h-12 bg-[#333A5C] text-center text-xl text-white rounded-md"
